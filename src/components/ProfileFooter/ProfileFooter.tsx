const ProfileFooter = ({
  ...
  return (
    <div className="flex flex-row justify-between items-center">
      <span>
        {showAllMode
          ? `Showing all matched profiles (${matched})`
          : `Total Count: ${total} / Matched Count: ${matched}`}
      </span>
      {!showAllMode && (
        <div className="flex flex-row gap-4">
          ...
          <span className="flex items-center justify-center gap-2">
            <input
              ...
              value={curPage}
              min={1}
              max={Math.max(1, Math.ceil(matched / limit))}
            ...
            />
            <span>/</span>
            <span>{Math.max(1, Math.ceil(matched / limit))}</span>
          </span>
          <button
            onClick={() => setCurPage(curPage + 1)}
            disabled={curPage === Math.ceil(matched / limit) || matched === 0}
            className="px-4 py-2 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-900 border border-gray-300 rounded-md hover:bg-gray-50 dark:hover:bg-gray-800 disabled:opacity-50"
          >
            Next
          </button>
